      ...
      combineLatest([this.theadChanged$, this.tbodyChanged$])
        .pipe(
          ...
          map(({ headRow, bodyRows }) => {
            return ({
              columnNames: headRow ? Array.from(headRow.children).map(headerCell => headerCell.textContent!):[],
              rows: bodyRows ? Array.from(bodyRows).map(row => [...Array.from(row.children)]) : []
            })
          }),
